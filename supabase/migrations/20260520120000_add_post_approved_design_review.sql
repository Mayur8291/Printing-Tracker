-- Sales review workflow for post-approved design images (Approved / Need Changes)
...
begin
  ...
  if actor_role = 'viewer' then
    ...
    if images_changed and not allow_approved_design_images then
      raise exception 'Viewer cannot edit approved design images';
    end if;

    if review_fields_changed and not images_changed and not allow_sales_review then
