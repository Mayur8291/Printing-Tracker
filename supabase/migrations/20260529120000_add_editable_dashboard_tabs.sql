-- Per-viewer edit access per dashboard tab (null = can edit all viewable tabs).

alter table public.profile_order_permissions
  add column if not exists editable_dashboard_tabs text[] default null;

comment on column public.profile_order_permissions.editable_dashboard_tabs is
  'When null, viewer may edit all tabs they can view. When set, only listed tab ids allow changes in that tab.';

create or replace function public.jwt_viewer_can_edit_dashboard_tab(tab_id text)
returns boolean
language sql
stable
security definer
...
as $$
  select public.jwt_user_is_admin()
  or not exists (
    select 1 from public.profile_order_permissions pop where pop.user_id = auth.uid()
  )
  or exists (
    select 1
    ...
    where pop.user_id = auth.uid()
      and (
        pop.allowed_dashboard_tabs is null
        or tab_id = any(pop.allowed_dashboard_tabs)
      )
      and (
        pop.editable_dashboard_tabs is null
        or tab_id = any(pop.editable_dashboard_tabs)
      )
  );
$$;

-- Patch order update trigger: billing invoice + dispatch verification need tab edit access.
...
begin
  ...
  if actor_role = 'viewer' then
    ...
    dispatch_fields_changed :=
      new.dispatch_sizes_verified is distinct from old.dispatch_sizes_verified
      or new.dispatch_sizes_qty_ok is distinct from old.dispatch_sizes_qty_ok
    ...
    if dispatch_fields_changed
       ...
       and (
         ...
         or (
           new.status in ('dispatch_fail', 'dispatched')
       ...
       and new.dispatch_sizes_verified is not distinct from old.dispatch_sizes_verified
    ...
    then
      if not public.jwt_viewer_can_edit_dashboard_tab('dispatch') then
        raise exception 'Viewer does not have edit access for the Dispatch tab';
      end if;
      return new;
    end if;
    ...
    if new.invoice_url is distinct from old.invoice_url
       ...
       and new.dispatch_verified_by is not distinct from old.dispatch_verified_by
    then
      if not public.jwt_viewer_can_edit_dashboard_tab('billing') then
        raise exception 'Viewer does not have edit access for the Billing tab';
      end if;
      return new;
    end if;
