-- Urgent delivery (same day or next day): new → pending after 10 minutes.
-- All other new jobs still promote after 12 hours.
...
-- Allow trigger/RPC to apply the 10-minute urgent path (status-only update).
