...
2d. **All PO Orders / PO History tables:** Same filter bar: **From** / **To** date pickers (inclusive on `po_date` / Dated), **Clear** (wipes From and To only), **Search** (exact trim + case-insensitive match on the voucher token after the last `/`, or the full PO Number, or supplier name, or coordinator name), **Coordinator** dropdown (names from generated rows, plus **All coordinators**), **View N / page** (numeric, default 10, saved in localStorage). Then columns **PO** (paper print + clickable **View PO**), **PO Number** (voucher value only), **Supplier** (bold R3 name), **Status** (All PO Orders: Pending / PO sent / PO Approved from the row; History: **Completed** only), **Coordinator**, **PO date** (Dated face), **Quantity** (C42). Status is display-only (backend updates Completed). View PO dialog shows the saved A4 print (**PURCHASE ORDER** heading + table). Its **Print** button uses the same rules as Create new PO Print: `window.print()`, full 210mm × 297mm sheet, print-only **C23**. Dialog chrome is hidden. Realtime refresh on `dashboard_purchase_orders`.
