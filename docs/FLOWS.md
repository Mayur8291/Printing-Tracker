...
2. **Entry:** `InternalSupportPlatformPanel` (shadcn Card). Not the Inventory **Support** tab (`enquiry`).
3. **Auth:** same as other Tools. Admin always sees it. Viewer needs `internal_support` in allowed tabs (or all-tabs null).
4. **Logic:** heading **Facing an issue? Select the option below that best describes your problem.** Then shadcn Buttons: Internet, Power Cut, Water Issue, Machinery, Food, Issue with Asset, Lift not working, Issue with Biometric, Floor Hygeine, Lost Personnal Belongings. Click toggles that option on/off. Many can stay selected. **Select your Floor** only appears if any picked issue needs a location (not Food / Issue with Asset / Issue with Biometric / Lost Personnal Belongings). Those four show **Comment** only. Mix (Food + Internet) shows Floor and requires it before Comment. Submit needs a non-empty comment. No ticket write yet; form clears; Alert: **Thank you. Your issue has been submitted and the concerned team will look into it shortly.**
5. **Failure:** empty comment → FieldError. Floor-required mix with no floor → Floor shown, Comment hidden, or FieldError on Submit. Zero issues → Floor and Comment hidden. Refresh clears picks, floor, and comment. Un-pick last floor-required issue → Floor hides. Un-pick floor when Floor is showing → Comment hides (text kept).
6. **Exit:** switch sidebar tab.
