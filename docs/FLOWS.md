...
2. **Entry:** `InternalSupportPlatformPanel` (shadcn Tabs + Card). Tabs **Raise an Issue**, **History**, and **Resolved**. Not the Inventory **Support** tab (`enquiry`).
3. **Auth:** same as other Tools. Admin always sees it. Viewer needs `internal_support` in allowed tabs (or all-tabs null). Submit needs a signed-in `profiles` row (`sessionUserId`). Name = `profileDisplayName(profile)`. History and Resolved: admin reads all rows. Non-admin reads only `raised_by = auth.uid()`. Status update is admin only (`jwt_user_is_admin()`) and only while status is not Resolved.
4. **Logic:** Tab **Raise an Issue** holds the form. Heading **Facing an issue? Select the option below that best describes your problem.** Then shadcn Buttons: Internet, Power Cut, Water Issue, Machinery, Food, Issue with Asset, Lift not working, Issue with Biometric, Floor Hygeine, Lost Personnal Belongings. Click toggles that option on/off. Many can stay selected. **Select your Floor** only appears if any picked issue needs a location (not Food / Issue with Asset / Issue with Biometric / Lost Personnal Belongings). Those four show **Comment** only. Mix (Food + Internet) shows Floor and requires it before Comment. Submit writes `internal_support_issues` (`status=Open`) then thank-you Alert. **History** lists Open / In Progress / Closed. **Resolved** lists `status = Resolved` (admin and non-admin). Admin Status Select on History can set Resolved; the row then leaves History. Resolved tab Status is a Badge only. Both list tabs: From, To, Clear, Search, View / page. **Name** filter is admin only. Table: **Issue** = underlined **View Issue** → View Issue dialog; **Name**; **Status**; **Date** = `created_at`. Newest first.
...
6. **Exit:** switch sidebar tab or Raise an Issue / History / Resolved.
