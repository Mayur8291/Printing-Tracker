...
| Team chat | `src/TeamChatPanel.jsx`, `src/teamChatService.js`, `src/dashboardPresence.js` | DMs, groups, org-wide channels (admin post only), attachments, GIFs, voice notes. Bottom inbox tabs Chats / Groups / Channels show unopened text-message counts. Presence: Online / Away / Offline (green / yellow / red). Same status on list dot and under the DM name. |
