...
| Internal Support Platform | `src/InternalSupportPlatformPanel.jsx` | Tools tab (`internal_support`, LifeBuoy). Issue Buttons; Floor only for location issues; Food/Asset/Biometric/Lost belongings → Comment only. Mix with other issues shows Floor. Separate from Support Enquiry. |
