...
| Asset Management | `src/AssetManagementPanel.jsx`, `src/hrAssetUtils.js` | Tools tab (`asset_management`). **Add asset** / **Save asset** writes staging `hr_assets`. **Assets** lists that register. Tags `IT-00001`+. Assign/check-in update the same row. Separate from Inventory SKUs. |
