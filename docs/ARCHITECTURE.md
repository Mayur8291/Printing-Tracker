...
| Asset Management | `AssetManagementPanel.jsx`, `hrAssetUtils.js` | Tools tab `asset_management`; Add/Save writes staging `hr_assets`; Assets list loads that table; assign/check-in update; not Inventory SKUs |
...
| Chat | `TeamChatPanel`, `teamChatService.js`, `teamChatReceipts.js`, `dashboardPresence.js` | Supabase + Storage. Presence heartbeat from App. Message actions via RPCs. Inbox tab badges sum unopened text `unread_count` by conversation kind. Voice notes upload as audio attachments. Channels are org-wide; only admins create and post. Delivery ticks on own DM/group posts use other members' `last_read_at` plus Online presence. Group admin RPCs mutate membership and `avatar_path`. Photos in `team-chat-group-avatars`. Incoming messages toast from `App.jsx` realtime with `sounds/chat-message.mp3`. |
