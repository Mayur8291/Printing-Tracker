...
| Internal Support Platform | `InternalSupportPlatformPanel.jsx`, `internalSupportIssueUtils.js` | Tools tab `internal_support`; Raise an Issue + History + Resolved; staging table `internal_support_issues`; History RLS own-or-admin; status update admin-only until Resolved; not Enquiry |
