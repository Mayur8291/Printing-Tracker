...
| Purchase Order | `PurchaseOrderPanel.jsx`, `PurchaseOrderLayoutGrid.jsx`, `PurchaseOrderHistoryTable.jsx`, `PurchaseOrderHistoryFilters.jsx`, `PurchaseOrderPrintSheet.jsx`, `purchaseOrderLayout.js`, `purchaseOrderVoucherUtils.js`, `purchaseOrderHistoryUtils.js`, `purchaseOrderAmountWords.js` | Main sidebar tab `purchase_order`; FileSpreadsheet icon; A4 unlabeled sheet; Generate PO stamps `dashboard_purchase_orders.generated_at` + snapshot; History filters + table; View PO = A4 heading + table print; print-only C23 signature |
