...
| Chat | `TeamChatPanel`, `teamChatService.js`, `dashboardPresence.js` | Supabase + Storage. Presence heartbeat from App. Message actions via RPCs. Inbox tab badges sum unopened text `unread_count` by conversation kind. Voice notes upload as audio attachments. |
