...
- **Production (2026-09-03):** `dashboard-stock-api` is deployed. Inbound NF key is set. `SCOTT_WEBHOOK_*` is **not** set on `levwrmvqdntngeasrtnb`, so live orders do not POST to the staging NF receiver. When they send a production webhook URL + secret, set those Edge secrets on production only and redeploy `dashboard-stock-api`.
