...
- **Team chat files** go to public bucket `team-chat-files`. The app no longer blocks by size (staging bucket cap 10 GB). Type list is images/video/audio/office/PDF/zip/txt. Download fetches the public URL as a blob on a user click, then uses the browser save dialog. Typed `http` links are not downloaded that way.
